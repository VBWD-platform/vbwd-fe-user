...
(function(){
  ...
  function avgGradientColor(gradStr){
    const hexRe=/#([0-9a-f]{6})\\b/gi;
    const rgbRe=/rgba?\\(([^)]+)\\)/gi;
    const cols=[]; let m;
    while((m=hexRe.exec(gradStr))){ const h=m[1]; cols.push([parseInt(h.slice(0,2),16),parseInt(h.slice(2,4),16),parseInt(h.slice(4,6),16),1]); }
    while((m=rgbRe.exec(gradStr))){ const p=m[1].split(',').map(x=>parseFloat(x.trim())); cols.push([p[0],p[1],p[2],p[3]===undefined?1:p[3]]); }
    if(!cols.length) return null;
    const n=cols.length;
    return [
      Math.round(cols.reduce((s,c)=>s+c[0],0)/n),
      Math.round(cols.reduce((s,c)=>s+c[1],0)/n),
      Math.round(cols.reduce((s,c)=>s+c[2],0)/n),
      1,
    ];
  }
  function effectiveBg(el){
    ...
    while(cur && cur!==document.documentElement){
      ...
      if(bgImg && bgImg!=='none' && /gradient/i.test(bgImg)){
        const avg=avgGradientColor(bgImg);
        if(avg) return avg;
      }
...
test('audit every theme × 4 pages', async ({ browser }) => {
  ...
  // Snapshot current default so we can restore after the audit
  const defaultProbe = await ctx.get(`${API_BASE}/api/v1/cms/styles/default`);
  const originalDefaultSlug = defaultProbe.ok()
    ? (await defaultProbe.json()).slug
    : null;
  ...
  try {
    ...
    // Restore the default style flag — nothing in this test mutates it,
    // but the populator may have been re-run between invocations and left
    // a different default; re-promote whatever we saw at the start.
    if (originalDefaultSlug) {
      const orig = styles.find((s) => s.slug === originalDefaultSlug);
      if (orig) {
        await ctx.post(
          `${API_BASE}/api/v1/admin/cms/styles/${orig.id}/default`,
          { headers: { Authorization: `Bearer ${token}` } },
        );
      }
    }
